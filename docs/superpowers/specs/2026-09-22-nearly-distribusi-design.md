...
| D12 | **`apps/api` tidak berubah** (kecuali env `HOST`, D13). Pesan push tanpa `channelId` diantar Expo ke kanal Android `default`; aplikasi membuat kanal itu sendiri (§4.2), jadi API tidak perlu diubah. |
| D13 | **Amandemen 2026-09-22 — VPS bersama, tanpa membeli domain.** VPS pemilik (`43.134.58.217`, Ubuntu 24.04) sudah menjalankan nginx di 80/443 dan layanan proyek lain. Karena itu: (a) nginx + certbot yang ada, bukan Caddy (`deploy/nginx/*.conf`, runbook §1.5b); (b) Node 24 terpisah di `/opt/node24`, `/usr/bin/node` v22 tidak disentuh; (c) ufw tidak diaktifkan — API mendengarkan hanya di `127.0.0.1` lewat env `HOST` baru (`bacaHost`, `apps/api/src/server-env.ts`), satu-satunya perubahan `apps/api` dan pengecualian atas D12; (d) nama host `api.nearly.43-134-58-217.sslip.io` dan `unduh.nearly.43-134-58-217.sslip.io` menggantikan domain yang dibeli (D3 ditunda). Pindah ke domain sendiri nanti cukup `server_name` + certbot + `eas update`. |
| D14 | **Amandemen 2026-09-22 — APK hanya `arm64-v8a`.** APK universal (4 ABI) berukuran 135 MB; lewat `expo-build-properties` (`android.buildArchs`) hanya `arm64-v8a` yang dibangun. Semua HP Android 64-bit (praktis semua HP sejak ±2017) terlayani; HP 32-bit murni (`armeabi-v7a`, sebagian Android Go lama) dan emulator x86 tidak bisa memasang APK ini. `expo-build-properties` adalah dependensi kedua fase ini, hanya plugin konfigurasi (tanpa kode runtime). Perubahan native → build ulang; `version` tetap 0.2.0 karena JS tidak berubah. |
